...
void send_prompt(GameState *game) {
    ...
    send(game->client_sockets[p], "PROMPT", 6, 0);
    send(game->client_sockets[o], "WAIT", 4, 0);
}
...
int check_winner(GameState *game) {
    // TODO: return 0 or 1 if winner, -1 if none
    return -1;
}
...
int main() {
    ...
    send(game.client_sockets[0], "SYMBOL:X", 8, 0);
    send(game.client_sockets[1], "SYMBOL:O", 8, 0);
    ...
    while (!game.game_over) {
        ...
        if (val <= 0) {
            ...
            send(game.client_sockets[1 - p], "DISCONNECT", 10, 0);
            break;
        }
        ...
        if (strncmp(buffer, "MOVE:", 5) == 0) {
            ...
            if (pos < 0 || pos > 8 || game.board[pos] != ' ') {
                send(game.client_sockets[p], "INVALID", 7, 0);
                continue;
            }
            ...
            if (w != -1) {
                char msg[64];
                sprintf(msg, "WINNER:%s", game.names[w]);
                send(game.client_sockets[0], msg, strlen(msg), 0);
                send(game.client_sockets[1], msg, strlen(msg), 0);
            ...
            if (check_draw(&game)) {
                send(game.client_sockets[0], "DRAW", 4, 0);
                send(game.client_sockets[1], "DRAW", 4, 0);
